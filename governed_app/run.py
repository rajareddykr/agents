...
def main() -> None:
    ...
    print(f"[governed_app] fin_agent  worker -> http://{HOST}:{FIN_PORT}")
    procs.append(_spawn("governed_ops.worker_app:app", FIN_PORT, "fin_agent"))

    print(f"[governed_app] res_agent  worker -> http://{HOST}:{RES_PORT}")
    ...
    print(f"[governed_app] coordinator      -> http://{HOST}:{COORD_PORT}")
    procs.append(_spawn(
    ...
    print(f"[governed_app] ready. POST http://{HOST}:{COORD_PORT}/mission")
        ...
        while True:
            for p in procs:
                if p.poll() is not None:
                    print(f"[governed_app] child exited ({p.returncode}); shutting down.")
                    _shutdown()
            time.sleep(1)
